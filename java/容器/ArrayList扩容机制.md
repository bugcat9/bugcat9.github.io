...
```java
     ...
     *当添加第一个元素时 长度自动扩充至DEFAULT_CAPACITY也就是10
     */
    public ArrayList() {
    ...
    public ArrayList(int initialCapacity) {
        ...
        } else if (initialCapacity == 0) {//初始容量等于0,区别于无参
            //创建空数组,后面添加一次元素容量扩充1
...
public boolean add(E e) {
   		//添加元素之前，先调用ensureCapacityInternal方法,确保内部容量
...
```java
 private static int calculateCapacity(Object[] elementData, int minCapacity) {
	...
	//调用了calculateCapacity
    private void ensureCapacityInternal(int minCapacity) {
        ensureExplicitCapacity(calculateCapacity(elementData, minCapacity));
    }
```

**当 要 add 进第1个元素时，minCapacity为1，在Math.max()方法比较后，minCapacity 为10。（如果使用有参数，length为0的情况，直接返回minCapacity）**
...
```java
    ...
    /**
     * ArrayList扩容的核心方法。
     * 注意理清oldCapacity、newCapacity、minCapacity这几个概念
     */
...
通过运行结果，我们可以看出向 ArrayList 添加大量元素之前最好先使用`ensureCapacity` 方法，以减少增量重新分配的次数，以节约时间。



**转载自：**[https://github.com/Snailclimb/JavaGuide/blob/master/docs/java/collection/ArrayList-Grow.md](https://github.com/Snailclimb/JavaGuide/blob/master/docs/java/collection/ArrayList-Grow.md)